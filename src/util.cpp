...
const char * const BITCOIN_CONF_FILENAME = "endorphin.conf";
const char * const BITCOIN_PID_FILENAME = "endorphind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "endorphin" is a composite category enabling all Endorphin-related debug output
            if(ptrCategory->count(string("endorphin"))) {
...
#else
    const char* pszModule = "endorphin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EndorphinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EndorphinCore
    // Mac: ~/Library/Application Support/EndorphinCore
    // Unix: ~/.endorphincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EndorphinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EndorphinCore";
#else
    // Unix
    return pathRet / ".endorphincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty endorphin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override endorphin.conf
